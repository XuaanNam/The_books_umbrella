    ...
    <div className="h-screen">
      ...
      <div className="bg-slate-100 pt-16">
        ...
        <div className="flex mx-5 pb-5 ">
          <div className="w-64 h-[800px] bg-white rounded drop-shadow-md">
          ...
          <div className="ml-3 p-2 w-[960px] bg-white rounded drop-shadow-lg">
            <div className="grid grid-cols-2 w-[280px] place-items-center p-5">
              <div className="font-medium text-slate-600">Sắp xếp theo</div>
              <Select
            ...
            <div className=" grid grid-cols-4">
              {data &&
                data?.map((product) => (
                  <div
                    key={product.id}
                    className="px-2 h-[330px] transition-all cursor-pointer mb-3"
                  >
                    <div className="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
                      ...
                      <div className="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-6 transition-all duration-500 group-hover:translate-y-0">
                        <p className="h-40 italic text-ellipsis overflow-hidden  text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                          {product.description}
                        </p>
                        <button
                          className="rounded-full bg-violet-600 hover:bg-violet-500 p-0.5 mb-2 px-3 font-com text-sm capitalize text-white shadow shadow-black/60"
                          onClick={() => Navigate(`/detail/${product.id}`)}
                        >
                          Xem thêm
                        </button>
                        <button
                          className="rounded-full bg-red-500 hover:bg-red-400 p-0.5 mb-2 px-3 font-com text-sm capitalize text-white shadow shadow-black/60"
